...
export default {
  ...
  mounted () {
    ...
    this.addScreenSpaceEventHandler(viewer)
  },
  methods: {
    initCesium: function () {
    ...
    addScreenSpaceEventHandler (viewer) {
      ...
      handler.setInputAction(function (event) {
        console.log('鼠标中键点击' + event.position)// 返回的是屏幕坐标
      }, Cesium.ScreenSpaceEventType.MIDDLE_CLICK)

      handler.setInputAction(function (event) {
        console.log('鼠标中键按下' + event.position)// 返回的是屏幕坐标
      }, Cesium.ScreenSpaceEventType.MIDDLE_DOWN)

      handler.setInputAction(function (event) {
        console.log('鼠标中键弹起' + event.position)// 返回的是屏幕坐标
      }, Cesium.ScreenSpaceEventType.MIDDLE_UP)

      // 获取事件上的回调函数
      let inputAction = handler.getInputAction(Cesium.ScreenSpaceEventType.MIDDLE_CLICK)
